...
$(function () {
    ...
    if (hasCache('noticiaLicitacao', idNoticia)) {
...
function restaurar() {
    var data = getDataCache('noticiaLicitacao', idNoticia);
...
function autosave() {
    ...
    cacheSave('noticiaLicitacao', data);
    modificado = true;
}

function removeCache() {
    removeData('noticiaLicitacao', idNoticia);
    modificado = false;
}
...
function validar() {
    ...
    if ($("#titulo").val() === "") {
        mensagem += "<li> O título da atualização é obrigatório.</li>";
    ...
    if (CKEDITOR.instances.texto.getData() === "") {
        mensagem += "<li> É obrigatório informar o texto da atualização.</li>";
